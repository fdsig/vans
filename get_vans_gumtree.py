...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('gumtree_scraping.log'),
...
MILEAGE_RE = re.compile(r"([\d,]+)\s*(?:miles?|mileage|k\s*miles?)", re.I)
YEAR_RE = re.compile(r"\b(19|20)\d{2}\b")
POSTCODE_RE = re.compile(r"\b[A-Z]{1,2}\d{1,2}[A-Z]?\s*\d[A-Z]{2}\b", re.I)
POSTED_DATE_RE = re.compile(r"(\d{1,2}\s+(?:minutes?|hours?|days?|weeks?)\s+ago)", re.I)

# ---------------------------------------------------------------------------
# Reuse PostcodeManager and ProxyRotator from eBay script
# ---------------------------------------------------------------------------
    ...
    try:
        ...
        for i, listing in enumerate(listing_elements):
            try:
                ...
                posted_date = await posted_elem.inner_text() if posted_elem else "N/A"
                ...
                seller_name = await seller_elem.inner_text() if seller_elem else "N/A"
                ...
                year_match = YEAR_RE.search(f"{title} {description}")
                year = int(year_match.group(0)) if year_match else None
                
                # Extract mileage from title or description
                mileage_match = MILEAGE_RE.search(f"{title} {description}")
                mileage = None
                if mileage_match:
                    try:
                        mileage_str = mileage_match.group(1).replace(',', '')
                        mileage = int(mileage_str)
                        # Convert if it looks like kilometers
                        if mileage > 500000:  # Likely in km
                            mileage = int(mileage / 1.609)  # Convert km to miles
                    except ValueError:
                        pass
                ...
                result = {
                    ...
                    'description': description[:500] if description != "N/A" else "N/A",  # Truncate
                    'location': location,
...
async def scrape_multiple_postcodes(postcodes: List[str], pages_per_postcode: int = 3, 
    ...
    """Scrape multiple postcodes concurrently for Gumtree listings"""
    ...
    try:
        while completed_postcodes < len(postcodes):
            ...
            logger.info(f"📋 Total Gumtree listings collected: {len(all_results)}")
    ...
    if all_results:
        ...
        logger.info(f"✅ Gumtree scraping completed: {len(df)} total listings")
        return df
    else:
...
def analyse(csv_path: Path, show_plots: bool = True) -> None:
    """Quick analysis of Gumtree listings"""
    ...
    try:
        ...
        if 'seller_type' in valid_price_df.columns:
            seller_stats = valid_price_df.groupby('seller_type')['price'].agg(['mean', 'count']).round(0)
            logger.info("🏪 Average asking price by seller type:")
            for seller_type, stats in seller_stats.iterrows():
                logger.info(f"   {seller_type}: £{stats['mean']:,.0f} ({stats['count']:.0f} listings)")
        
        # Featured listings analysis
        if 'is_featured' in valid_price_df.columns:
            featured_stats = valid_price_df.groupby('is_featured')['price'].agg(['mean', 'count']).round(0)
            logger.info("⭐ Average asking price by featured status:")
            for is_featured, stats in featured_stats.iterrows():
                status = "Featured" if is_featured else "Regular"
                logger.info(f"   {status}: £{stats['mean']:,.0f} ({stats['count']:.0f} listings)")
        ...
        if show_plots and plt is not None and np is not None:
            try:
                ...
                fig.suptitle('Ford Transit L2 H2 Listings Analysis (Gumtree UK)', fontsize=16)
                
                # Price distribution
                axes[0, 0].hist(valid_price_df['price'], bins=30, alpha=0.7, color='lightgreen', edgecolor='black')
                ...
                if 'year' in valid_price_df.columns:
                    ...
                    if not year_price_df.empty:
                        axes[0, 1].scatter(year_price_df['year'], year_price_df['price'], alpha=0.6, color='orange')
                ...
                if 'mileage' in valid_price_df.columns:
                    ...
                    if not mileage_price_df.empty and len(mileage_price_df) > 1:
                        axes[1, 0].scatter(mileage_price_df['mileage'], mileage_price_df['price'], alpha=0.6, color='purple')
                ...
                if 'seller_type' in valid_price_df.columns:
                    valid_price_df.boxplot(column='price', by='seller_type', ax=axes[1, 1])
                    axes[1, 1].set_title('Asking Price by Seller Type')
...
Examples:
  ...
  python get_vans_gumtree.py analyse transit_gumtree.csv
    ...
    scrape_parser = subparsers.add_parser('scrape', help='Scrape Gumtree listings for single postcode')
    ...
    scrape_parser.add_argument('--outfile', type=Path, default=Path('gumtree_listings.csv'), help='Output CSV file')
    ...
    multi_parser.add_argument('--outfile', type=Path, default=Path('gumtree_multi.csv'), help='Output CSV file')
    ...
    uk_parser.add_argument('--outfile', type=Path, default=Path('ford_transit_uk_gumtree.csv'), help='Output CSV file')
