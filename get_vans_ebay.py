...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('ebay_scraping.log'),
...
class PostcodeManager:
    ...
    def record_success_rate(self, postcode: str, listings_found: int):
        ...
        logger.info(f"Recorded success rate for {postcode}: {success_rate:.1%} ({listings_found} listings)")
...
def parse_args() -> argparse.Namespace:
    ...
    scrape_parser.add_argument("--postcode", required=True, help="UK postcode to scrape")
    scrape_parser.add_argument("--pages", type=int, default=10, help="Number of pages to scrape")
    scrape_parser.add_argument("--outfile", type=Path, default="ebay_transit_listings.csv", help="Output CSV file")
    scrape_parser.add_argument("--max-price", type=int, default=100000, help="Maximum price filter")
    
    # Scrape-multi command (multiple postcodes)
    multi_parser = subparsers.add_parser("scrape-multi", help="Scrape multiple postcodes with strategies")
    ...
    multi_parser.add_argument("--postcode-limit", type=int, default=50, help="Number of postcodes to use")
    multi_parser.add_argument("--pages-per-postcode", type=int, default=5, help="Pages per postcode")
    multi_parser.add_argument("--postcodes", nargs="+", help="Custom postcodes list")
    multi_parser.add_argument("--proxy-file", type=Path, help="File containing proxy list")
    multi_parser.add_argument("--outfile", type=Path, default="ebay_transit_multi.csv", help="Output CSV file")
    ...
    multi_parser.add_argument("--exclude-used", action="store_true", help="Exclude previously used postcodes")
    
    # Scrape-uk command (optimized UK-wide)
    uk_parser = subparsers.add_parser("scrape-uk", help="Optimized UK-wide scraping")
    uk_parser.add_argument("--outfile", type=Path, default="ebay_ford_transit_uk_complete.csv", help="Output CSV file")
    ...
    uk_parser.add_argument("--proxy-file", type=Path, help="Proxy file")
    uk_parser.add_argument("--max-browsers", type=int, default=5, help="Max concurrent browsers")
    uk_parser.add_argument("--max-pages", type=int, default=25, help="Max pages per postcode")
...
def main():
    ...
    if args.command == "scrape":
        asyncio.run(scrape_ebay_single_postcode(args.postcode, args.pages, args.outfile, args.max_price))
