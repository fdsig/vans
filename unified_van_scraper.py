...
class UnifiedVanScraper:
    ...
    def __init__(self, output_file: str = "ford_transit_uk_complete.csv"):
        ...
        self.scrapers = {
            ...
            'cargurus': {
                'script': 'get_vans_cargurus.py', 
                'command': 'scrape-uk',
                'source_name': 'cargurus'
            },
            'ebay': {
                'script': 'get_vans_ebay.py',
                'command': 'scrape-uk', 
                'source_name': 'ebay'
            },
            'gumtree': {
                'script': 'get_vans_gumtree.py',
                'command': 'scrape-uk',
                'source_name': 'gumtree'
            },
            'facebook': {
                'script': 'get_vans_facebook.py',
                'command': 'scrape-uk',
    ...
    def build_scraper_command(self, scraper_config: Dict[str, str], args: argparse.Namespace) -> List[str]:
        ...
        # Temporary output file for this source
        temp_output = f"temp_{scraper_config['source_name']}_results.csv"
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_parallel) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_scraper):
                ...
                try:
                    ...
                    if result['success']:
                        temp_file = f"temp_{scraper_name}_results.csv"
