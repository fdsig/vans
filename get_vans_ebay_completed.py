...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('ebay_completed_scraping.log'),
...
Examples:
  ...
  python get_vans_ebay_completed.py analyse transit_completed.csv
    ...
    scrape_parser = subparsers.add_parser('scrape', help='Scrape completed listings for single postcode')
    ...
    scrape_parser.add_argument('--outfile', type=Path, default=Path('ebay_completed_listings.csv'), help='Output CSV file')
    ...
    multi_parser.add_argument('--outfile', type=Path, default=Path('ebay_multi_completed.csv'), help='Output CSV file')
    ...
    uk_parser.add_argument('--outfile', type=Path, default=Path('ford_transit_uk_completed_ebay.csv'), help='Output CSV file')
    ...
    uk_parser.add_argument('--max-pages', type=int, default=20, help='Max concurrent pages')
    ...
    try:
        if args.command == 'scrape':
            asyncio.run(scrape_ebay_single_postcode(
                args.postcode, args.pages, args.outfile, args.max_price
            ))
